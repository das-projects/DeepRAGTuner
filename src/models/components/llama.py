...
def remap_state_dict_hf_llama(
    ...
    def key_mapping_emb(key):
        return re.sub(
            r"^model.embed_tokens.", "transformer.embeddings.word_embeddings.", key
        )
    ...
    def inv_permute(w):
        ...
        return rearrange(
            w,
            "(h two d) n -> (h d two) n",
            d=config.n_embd // config.n_head // 2,
            two=2,
        )
...
def inv_remap_state_dict_hf_llama(
    ...
    def key_mapping_emb(key):
        return re.sub(
            r"^transformer.embeddings.word_embeddings.", "model.embed_tokens.", key
        )
