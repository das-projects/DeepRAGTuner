...
def remap_state_dict_hf_gptj(state_dict, config):
    ...
    def key_mapping_emb(key):
        return re.sub(
            r"^transformer.wte.", "transformer.embeddings.word_embeddings.", key
        )
    ...
    def key_mapping_ln(key):
        return re.sub(
            r"^transformer.layers.(\d+).ln_1.", r"transformer.layers.\1.norm1.", key
        )
    ...
    def key_mapping_mlp(key):
        key = re.sub(
            r"^transformer.layers.(\d+).mlp.fc_in.",
            r"transformer.layers.\1.mlp.fc1.",
            key,
        )
        key = re.sub(
            r"^transformer.layers.(\d+).mlp.fc_out.",
