...
def remap_state_dict_hf_bigcode(state_dict, config: PretrainedConfig):
    ...
    def key_mapping_pos_emb(key):
        return re.sub(
            r"^transformer.wpe.", "transformer.embeddings.position_embeddings.", key
        )
    ...
    for d in range(config.num_hidden_layers):
        ...
        state_dict[f"transformer.layers.{d}.mixer.Wqkv.weight"] = torch.cat(
            (q, k, v), dim=0
        )
        ...
        state_dict[f"transformer.layers.{d}.mixer.Wqkv.bias"] = torch.cat(
            (q, k, v), dim=0
        )
...
def inv_remap_state_dict_hf_bigcode(state_dict, config: PretrainedConfig):
    ...
    def inv_key_mapping_pos_emb(key):
        return re.sub(
            r"^transformer.embeddings.position_embeddings.", "transformer.wpe.", key
        )

    state_dict = OrderedDict(
        (inv_key_mapping_pos_emb(k), v) for k, v in state_dict.items()
    )
    ...
    for d in range(config.num_hidden_layers):
        ...
        q, k, v = torch.split(
            Wqkv_weight,
            [embed_dim, head_dim * config.n_head, head_dim * config.n_head],
            dim=0,
        )
    ...
    state_dict = OrderedDict(
        (inv_key_mapping_attn(k), v) for k, v in state_dict.items()
    )
