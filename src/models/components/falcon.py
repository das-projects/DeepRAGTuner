...
def remap_state_dict_hf_falcon(state_dict, config):
    ...
    def key_mapping_ln(key):
        ...
        key = re.sub(
            r"^transformer.layers.(\d+).ln_attn.", r"transformer.layers.\1.norm1.", key
        )
        key = re.sub(
            r"^transformer.layers.(\d+).ln_mlp.", r"transformer.layers.\1.norm2.", key
        )
        return key
    ...
    def key_mapping_mlp(key):
        key = re.sub(
            r"^transformer.layers.(\d+).mlp.dense_h_to_4h.",
            r"transformer.layers.\1.mlp.fc1.",
            key,
        )
        key = re.sub(
            r"^transformer.layers.(\d+).mlp.dense_4h_to_h.",
    ...
    for l in range(config.n_layer):
        ...
        Wq = rearrange(
            Wqkv[:, :-2], "group ratio headdim ... -> (group ratio headdim) ..."
        )
        Wk = rearrange(
            Wqkv[:, [-2]], "group ratio headdim ... -> (group ratio headdim) ..."
        )
        Wv = rearrange(
            Wqkv[:, [-1]], "group ratio headdim ... -> (group ratio headdim) ..."
        )
